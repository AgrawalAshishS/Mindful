/*
 *
 *  * Copyright (c) 2024 Mindful (https://github.com/akaMrNagar/Mindful)
 *  * Author : Pawan Nagar (https://github.com/akaMrNagar)
 *  *
 *  * This source code is licensed under the GPL-2.0 license license found in the
 *  * LICENSE file in the root directory of this source tree.
 *
 */

import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:mindful/core/database/app_database.dart';
import 'package:mindful/core/services/drift_db_service.dart';

final schedulesProvider = StateNotifierProvider<SchedulesNotifier, List<Schedule>>(
  (ref) => SchedulesNotifier(),
);

class SchedulesNotifier extends StateNotifier<List<Schedule>> {
  late final _dao = DriftDbService.instance.driftDb.dynamicRecordsDao;

  SchedulesNotifier() : super([]) {
    _init();
  }

  void _init() async {
    state = await _dao.fetchSchedules();
  }

  Future<void> addSchedule(SchedulesCompanion schedule) async {
    await _dao.insertSchedule(schedule);
    state = await _dao.fetchSchedules();
  }

  Future<void> updateSchedule(Schedule schedule) async {
    await _dao.updateSchedule(schedule);
    state = await _dao.fetchSchedules();
  }

  Future<void> deleteSchedule(int id) async {
    await _dao.deleteSchedule(id);
    state = state.where((schedule) => schedule.id != id).toList();
  }
}
